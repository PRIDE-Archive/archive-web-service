...
//    }
    ...
    // Protein map methods
    public static List<ProteinDetail> mapProteinIdentifiedListToWSProteinDetailList(Collection<ProteinIdentified> proteins) {
        if (proteins == null) { return null; }
        if (proteins.isEmpty()) { return new ArrayList<ProteinDetail>(0); }

        List<ProteinDetail> mappedObjects = new ArrayList<ProteinDetail>();
        for (ProteinIdentified proteinIdentified : proteins) {
            ...
            mappedObject.setProjectAccessions(proteinIdentified.getProjectAccessions());
            mappedObject.setAssayAccessions(proteinIdentified.getAssayAccessions());
    ...
    public static List<PsmDetail> mapPsmListToWSPsmDetailList(List<Psm> psms) {
        ...
        for (Psm psm : psms) {
            ...
            mappedObject.setSearchEngine(psm.getSearchEngine());
            mappedObject.setSearchEngineScore(psm.getSearchEngineScore());
            mappedObject.setSpectrumID( psm.getSpectrumId() );
            mappedObject.setId(psm.getId());
            mappedObject.setReportedID( psm.getReportedId() );
            mappedObject.setModifications( psm.getModifications() );
            mappedObjects.add(mappedObject);
        }
